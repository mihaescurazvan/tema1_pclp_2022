#!/bin/bash
EXEC=problema
TESTS_NO_PROBLEMS=(20 15 20 20)
PROBLEMS_NO=4
TOTAL=0
WORKING_DIR=`pwd`
array_points=(0 0 0 0 0)
max_array_points=(0 20 30 40 40)
make
...
function run_tests_for_problem_4() {
    PROBLEM_NO=$1
    NO_TESTS=${TESTS_NO_PROBLEMS[$((PROBLEM_NO-1))]}
    ...
    for ((i=1;i<=$NO_TESTS;i++)); do
		...
		test_output="$output/test$i.out"
	    test_ref="$ref/test$i.ref"
        check_test_4 $test_input $test_output $test_ref $i $PROBLEM_NO
    done
    echo "====== You got ${array_points[$PROBLEM_NO]} out of ${max_array_points[$PROBLEM_NO]} for $PROBLEM_NO ======="
